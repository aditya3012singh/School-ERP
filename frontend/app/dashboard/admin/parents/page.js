...
import { fetchParents } from "@/store/api/admin.thunk";
  ...
  return (
    <div className="bg-gray-50 p-8 min-h-screen">
      <div className="mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-1">Parents</h1>
          <p className="text-gray-600">
            View parent profiles and linked student information.
          </p>
        </div>
        ...
        {!loading && error && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
            <p className="text-red-600">Error loading parents: {error}</p>
          </div>
        )}
...
function ParentTableSkeleton({ rows = 6 }) {
  return (
    <div className="bg-white rounded-lg shadow-sm overflow-hidden animate-pulse">
      <div className="bg-gray-50 border-b px-6 py-4 grid grid-cols-4 gap-4">
        {[1, 2, 3, 4].map((i) => (
      ...
      {[...Array(rows)].map((_, idx) => (
        <div
          key={idx}
          className="px-6 py-4 grid grid-cols-4 gap-4 border-b"
        >
          <div className="flex items-center gap-3">
