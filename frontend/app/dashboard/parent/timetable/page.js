...
import { fetchChildTimetable } from "@/store/api/parent.thunk";
import { Clock, AlertCircle, BookOpen } from "lucide-react";

export default function ParentTimetable() {
  const dispatch = useDispatch();
  const { timetable, loading, error } = useSelector((state) => state.parent);

  useEffect(() => {
    dispatch(fetchChildTimetable());
  }, [dispatch]);
  ...
  return (
    <div className="p-6 space-y-6 w-full">
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white p-6 rounded-lg shadow-lg">
        ...
        <p className="text-indigo-100 mt-1">View your child&apos;s weekly schedule</p>
      </div>
          ...
          return (
            <div key={day} className="bg-white rounded-lg shadow overflow-hidden">
              <div className="bg-indigo-50 px-6 py-3 border-b border-indigo-100">
                <h3 className="font-semibold text-lg text-indigo-900">{day}</h3>
              </div>
