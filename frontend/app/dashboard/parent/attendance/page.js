...
import { fetchChildAttendance } from "@/store/api/parent.thunk";
import { CalendarCheck, AlertCircle } from "lucide-react";

export default function ParentAttendance() {
  const dispatch = useDispatch();
  const { attendance, loading, error } = useSelector((state) => state.parent);
  ...
  return (
    <div className="p-6 space-y-6 w-full">
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white p-6 rounded-lg shadow-lg">
        ...
        <p className="text-indigo-100 mt-1">Track your child&apos;s attendance records</p>
      </div>
